/**
 * Function to change the image on clicking a content-box
 * @param {HTMLElement} element - The clicked content-box element
 * @param {string} imageSrc - The image source to change to
 * Author: Sukanto Chatterjee @ Cybogeek
 * https://github.com/Cybogeek/Fyle-Webpage
 */
function changeImage(element, imageSrc) {
    // Remove active-content class from all content-boxes
    const contentBoxes = document.querySelectorAll('.content-box');
    contentBoxes.forEach(box => {
        box.classList.remove('active-content');
    });

    // Add active-content class to the clicked content-box
